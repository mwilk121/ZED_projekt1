 ...
 * redukcja danuch opisujacych pojedynczego pacjenta do jednego wiersza, z zachowaniem najwczesniejszych wartosci atrybutow (żeby przewidziec smierc pacjenta jak najwczesniej)
 * usuniecie kolumn niebedacych atrybutami
 * podzial na zbiory uczacy i testujacy
 * usuniecie wierszy i kolumn, w ktorych wartosci brakujace to wiecej niz 40% danych
 * przypisanie wartosci sredniej atrybutow do wartosci brakujacych
 
```{r preparing_data, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
ml_data<- cleaned%>%group_by(patient_id)%>%summarise_all(funs(first))
ml_data$outcome=factor(ml_data$outcome, labels = make.names(c("negative", "positive")))
ml_data<-ml_data[-(1:6)]
ml_data<-ml_data[which(rowMeans(!is.na(ml_data)) > 0.4), which(colMeans(!is.na(ml_data)) > 0.4)]
...
```{r classificator, eval=TRUE, warning=FALSE, message=FALSE}
